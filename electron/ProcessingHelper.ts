...
export class ProcessingHelper {
 ...
 try{
          ...
          console.error("Error using Gemini API: line 569", error);
          if (error?.status === 429 || error?.message?.includes('429')) {
            return {
              success: false,
              error: "Gemini API rate limit exceeded. You may be on the free tier — please wait a minute and try again, or upgrade your Gemini plan."
            };
          }
...
`;
      ...
      if (config.apiProvider === "openai") {
        ...
        try {
          ...
          const solutionResponse = await this.geminiClient.chat.completions.create({
            model: config.solutionModel || "gemini-2.5-flash",
            messages: [
              { role: "system", content: "You are an expert coding interview assistant. Provide clear, optimal solutions with detailed explanations." },
              { role: "user", content: promptText }
        ...
        try {
          const solutionResponse = await this.anthropicClient.chat.completions.create({
...
4. A clear explanation of the changes needed`
      ...
      if (config.apiProvider === "openai") {
        ...
        try {
          ...
          const debugResponse = await this.geminiClient.chat.completions.create({
        ...
        try {
          ...
          const debugResponse = await this.anthropicClient.chat.completions.create({
