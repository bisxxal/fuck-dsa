...
export class ScreenshotHelper {
  ...
  constructor(view: "queue" | "solutions" | "debug" = "queue") {
    ...
    this.tempDir = path.join(app.getPath("temp"), "screenshots");
  ...
  //   replaces loadExistingScreenshots() to ensure we start with empty queues
  private cleanScreenshotDirectories(): void {
    try {
      // Clean main screenshots directory
      if (fs.existsSync(this.screenshotDir)) {
        const files = fs.readdirSync(this.screenshotDir)
          .filter((file) => file.endsWith(".png")).map((file) => path.join(this.screenshotDir, file));
      ...
      console.error("Error cleaning screenshot directories:", error);
    }
  }
